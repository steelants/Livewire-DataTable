                                ...
                                <td>
                                    @foreach ($this->actions($row) as $action)
                                        ...
                                        @elseif ($action['type'] == 'livewire')
                                            <button @if ($action['is_danger'] ?? false) onclick="return confirm('datatables.action.configmation')" @endif class="btn ms-1 @if ($action['is_danger'] ?? false) btn-danger @else btn-primary @endif" wire:click='{{ $action['action'] }}({{ $action['parameters'] }})'>{{ __($action['name']) }}</button>
                                        @else
